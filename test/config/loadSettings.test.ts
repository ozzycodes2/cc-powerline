...
import { loadSettings, settingsWarnings } from '../../src/config/loadSettings.js';
...
describe('settingsWarnings', () => {
  it('warns when builtin style has right-side widgets', () => {
    const s: Settings = { style: 'builtin', lines: [{ left: [], right: [{ type: 'model' }] }] };
  ...
  it('is silent for builtin with only left widgets', () => {
    const s: Settings = { style: 'builtin', lines: [{ left: [{ type: 'model' }], right: [] }] };
    expect(settingsWarnings(s)).toEqual([]);
  });

  it('is silent for powerline with right widgets', () => {
    const s: Settings = { style: 'powerline', lines: [{ left: [], right: [{ type: 'model' }] }] };
    expect(settingsWarnings(s)).toEqual([]);
  });
});
...
describe('loadSettings', () => {
  ...
  it('returns defaults on malformed JSON', async () => {
    const res = await loadSettings({ path: '/x', readText: read('{ not json') });
    expect(res.source).toBe('default');
  });

  it('parses a valid file and carries warnings', async () => {
    const file = JSON.stringify({ style: 'builtin', lines: [{ left: [], right: [{ type: 'model' }] }] });
  ...
  it('falls back to default lines when the config omits them', async () => {
    const res = await loadSettings({ path: '/x', readText: read(JSON.stringify({ style: 'builtin' })) });
