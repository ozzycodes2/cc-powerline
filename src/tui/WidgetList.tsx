  ...
  return (
    <Box flexDirection="column" marginRight={4}>
      ...
      {items.length === 0 ? (
        ...
        <SelectList
          items={items.map((w) => ({ label: w.type }))}
          cursor={cursor}
        />
      )}
    </Box>
...
export function WidgetList({ state, dispatch }: WidgetListProps) {
  ...
  useInput((input, key) => {
    ...
    if (moveMode) {
      if (dir === 'up')
        dispatch({ type: 'MOVE_WIDGET', lineIndex, side, itemIndex, dir: -1 });
      else if (dir === 'down')
        dispatch({ type: 'MOVE_WIDGET', lineIndex, side, itemIndex, dir: 1 });
    ...
    if (dir === 'up' || dir === 'down') {
      dispatch({
        type: 'NAVIGATE',
        focus: { itemIndex: moveCursor(itemIndex, dir, group.length) },
      });
    } else if (dir === 'left' || dir === 'right' || key.tab) {
      ...
      if (group.length > 0)
        dispatch({ type: 'REMOVE_WIDGET', lineIndex, side, itemIndex });
    } else if (input === 'm') {
    ...
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        <Group
          title="left"
          items={line?.left ?? []}
          cursor={side === 'left' ? itemIndex : -1}
        />
        <Group
          title="right"
          items={line?.right ?? []}
          cursor={side === 'right' ? itemIndex : -1}
        />
      </Box>
    </Box>
  );
